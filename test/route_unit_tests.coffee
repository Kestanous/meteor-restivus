describe 'A route', =>
  ...
  it 'can be constructed with options', (test) =>
    route = new Restivus.Route Api, 'test-route-1', {authRequired: true, roleRequired: ['admin', 'dev']},
  ...
  it 'can be constructed without options', (test) =>
    route = new Restivus.Route Api, 'test-route-2',
  ...
  it 'should support endpoints for all HTTP methods', (test) =>
    route = new Restivus.Route Api, 'test-route-3',
      get: -> 'GET test-route-2'
